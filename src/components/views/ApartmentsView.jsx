    ...
    <div className="flex-1 min-h-0 flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {paginatedApartments.map((apt) => {
          ...
          return (
          <div key={apt.id} className={`bg-white dark:bg-slate-900 rounded-lg shadow-sm border ${isNotClean ? 'border-gray-100 dark:border-slate-800 border-r-4 border-r-amber-500' : 'border-gray-100 dark:border-slate-800'} flex flex-col h-full relative group transition-all hover:shadow-md overflow-hidden`}>
            {/* Top Half: Photo */}
            <div
            ...
            <div className="mt-auto flex items-end justify-between pt-3 border-t border-gray-50 dark:border-slate-800">
              <div>
                ...
                <p className="text-xl font-semibold text-ink dark:text-green-400">{apt.basePrice} <span className="text-xs text-gray-400 font-bold">ر.س / ليلة</span></p>
              </div>
              {isNotClean && (
                <button
                  onClick={() => handleToggleCleaningStatus(apt)}
                  className="text-xs font-bold text-ink  bg-canvas hover:bg-surface-soft px-3 py-1.5 rounded-lg transition-colors"
                >
                  تم التنظيف
                </button>
              )}
        ...
        {(user?.role === 'admin' || user?.permissions?.canEdit) && (
          <button
            ...
            className="border-2 border-dashed border-gray-200 dark:border-slate-700 rounded-lg p-6 flex flex-col items-center justify-center text-gray-400 hover:border-blue-400 hover:bg-blue-50/50 hover:text-blue-600 dark:hover:bg-slate-800 transition-all cursor-pointer bg-transparent min-h-[200px]"
          >
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    {formData.cleaningType === 'per_booking' && (
                        <input type="number" placeholder="تكلفة التنظيف للحجز الواحد (مثال: 50)" className="w-full px-4 py-2.5 border border-gray-200 dark:border-slate-700 rounded-md outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-slate-800 dark:text-slate-100 transition-all" value={formData.cleaningCost} onChange={(e) => setFormData({...formData, cleaningCost: e.target.value})} />
                    )}
