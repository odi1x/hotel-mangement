  ...
  return (
    <div className="w-full select-none">
      <div className="flex items-center justify-between mb-2">
        <button type="button" onClick={() => setView(new Date(y, m - 1, 1))} className="icon-action p-1.5" aria-label="الشهر السابق"><ChevronRight size={18} /></button>
        <span className="text-base font-semibold text-ink dark:text-white">{AR_MONTHS[m]} {y}</span>
        <button type="button" onClick={() => setView(new Date(y, m + 1, 1))} className="icon-action p-1.5" aria-label="الشهر التالي"><ChevronLeft size={18} /></button>
      </div>
              ...
              <button
                ...
                className={`w-10 h-10 rounded-full flex items-center justify-center text-sm transition-colors
                  ${endpoint ? 'bg-accent text-white font-semibold'
                    ...
                    : 'text-body dark:text-[#a1a1aa] hover:bg-surface-soft dark:hover:bg-surface-dark-elevated'}`}
              >
                {d.getDate()}
              </button>
            </div>
      ...
      <div className="mt-4 pt-3 border-t border-hairline-soft dark:border-[#242424]">
        {sN != null ? (
          <div className="flex items-center justify-center gap-3 text-sm">
            <div className="text-center">
              <div className="text-[10px] text-muted-soft mb-0.5">الوصول</div>
              <div className="font-semibold text-ink dark:text-white">{start.getDate()} {AR_MONTHS[start.getMonth()]}</div>
            </div>
            <span className="text-muted-soft">←</span>
            <div className="text-center">
              <div className="text-[10px] text-muted-soft mb-0.5">المغادرة</div>
