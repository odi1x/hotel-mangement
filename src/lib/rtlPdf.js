...
//     the connecting forms must already be encoded; Amiri carries every FB50
//     —FEFF form in its cmap, so the glyphs map 1:1.
...
//     internal LTR order by staying whole tokens.
...
export function processRTL(value, fallback = '') {
  ...
  // 2) Reverse token order, keeping each token intact. `split` keeps the
  //    single spaces between tokens so spacing is preserved after reversal.
